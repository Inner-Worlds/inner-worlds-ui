...
const App = () => {
  ...
  const [getUser, { client, loading, error }] = useLazyQuery(GET_USER, {
  ...
  useEffect(() => {
    if (user.id && history.location.pathname !== "/dreams") {
      ...
      !user.id &&
      (history.location.pathname === "/Dreams" ||
    ...
    <div className="App">
      <Switch>
        <Route
          ...
          path="/"
          render={() => (
            <Login loginUser={getUser} tryLogin={tryLogin} error={error} />
          )}
        />
        <Route
          exact
          ...
          render={() => (
            <>
              ...
              <DreamList
                userID={user.id}
                dreams={user.dreams}
                deleteDream={deleteSingleDream}
                updateDream={updateSingleDream}
                updateEmotionsAndTags={updateEmotionsAndTags}
                setEditing={setCurrentlyEditing}
                currentlyEditing={currentlyEditing}
                deleteCardError={deleteError}
                saveError={saveError}
              />
            </>
          )}
